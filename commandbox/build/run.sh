...
if [[ $USER ]] && [[ $USER != $(whoami) ]]; then
	...
	if [[ $ADMIN_PASSWORD_SET != true ]]; then
		if [[ ${SERVER_HOME_DIRECTORY} == "${HOME}/serverHome" ]]; then
			...
			echo "WARN: Configuration did not detect any known mechanisms for changing the default password.  Your CF engine administrative password has been set to a random value."
